...
public class LinkJSONSerializer extends JsonSerializer<Link> {
    ...
    public void serialize(Link link, JsonGenerator jGen, SerializerProvider sProvider)
        ...
        jGen.writeStringField("sourceSwitch", link.getSrc().toString());
        jGen.writeNumberField("sourcePort", link.getSrcPort().getPortNumber());
        jGen.writeStringField("destinationSwitch", link.getDst().toString());
        jGen.writeNumberField("destinationPort", link.getDstPort().getPortNumber());
