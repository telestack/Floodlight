...
import it.garr.ccbalancer.ICCBalancerListener;
...
public class TopologyManagerCCBalancer implements IFloodlightModule, ITopologyService, IRoutingService, ILinkDiscoveryListener, IOFMessageListener, ICCBalancerListener {
	...
	protected TopologyInstanceCCBalancer currentInstance;
	protected TopologyInstanceCCBalancer currentInstanceWithoutTunnels;
	...
	public boolean isAttachmentPointPort(DatapathId switchid, OFPort port, boolean tunnelEnabled) {
		...
		TopologyInstanceCCBalancer ti = getCurrentInstance(tunnelEnabled);
	...
	private class HAListenerDelegate implements IHAListener {
		...
		@Override
		public String getName() {
			return TopologyManagerCCBalancer.this.getName();
		}
	...
	protected void doFloodBDDP(DatapathId pinSwitch, OFPacketIn pi,
		...
		TopologyInstanceCCBalancer ti = getCurrentInstance(false);
	...
	protected boolean createNewInstance(String reason) {
		...
		TopologyInstanceCCBalancer nt = new TopologyInstanceCCBalancer(switchPorts,
				blockedPorts,
				openflowLinks,
				broadcastDomainPorts,
				tunnelPorts, mpbalance);
		nt.compute();
	...
	public TopologyInstanceCCBalancer getCurrentInstance(boolean tunnelEnabled) {
		if (tunnelEnabled)
			return currentInstance;
		else return this.currentInstanceWithoutTunnels;
	}

	public TopologyInstanceCCBalancer getCurrentInstance() {
	...
	@Override
	public Route getElephantRoute(DatapathId srcId, OFPort srcPort,
			DatapathId dstId, OFPort dstPort) {
		TopologyInstanceCCBalancer ti=getCurrentInstance();
		return ti.getElephantRoute(srcId, dstId);
	}
	...
	public void costChanged() {
		try {
            ...
            if (result == true) log.trace("Topology update Success");
            else log.trace("Topology update Failed");
        }
        catch (Exception e) {
            log.error("Error in topology instance task thread", e);
        }
	...
	protected boolean createNewCostInstance() throws IOException {
        ...
        Map<NodePortTuple, Set<Link>> openflowLinks = new HashMap<NodePortTuple, Set<Link>>(switchPortLinks);
